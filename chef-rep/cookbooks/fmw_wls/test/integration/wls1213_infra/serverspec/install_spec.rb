...
describe 'fmw_wls::install' do
  ...
  describe file('/usr/java/jdk1.8.0_171') do
    it { should be_directory }
    it { should be_owned_by 'root' }
  end

  describe file('/usr/java/jdk1.8.0_171/bin/java') do
    it { should be_file }
    it { should be_owned_by 'root' }
  ...
  if ['redhat'].include?(os[:family]) and os[:release] >= '7.0'
...
=begin
  describe file('/opt/oracle/middleware_1221/wlserver') do
  it { should be_directory }
  it { should be_owned_by 'oracle' }
  it { should be_grouped_into 'oinstall' }
end
